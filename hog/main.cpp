...
static string posSamplesDir = "../training_data/positives/";
// Directory containing negative sample images
static string negSamplesDir = "../training_data/negatives/";
...
static const Size winStride = Size(5, 5);
// </editor-fold>
...
static void showDetections(const vector<Point>& found, Mat& imageData) {
    ...
    for (i = 0; i < found.size(); ++i) {
        ...
        rectangle(imageData, Rect(r.x-16, r.y-32, 32, 64), Scalar(255, 0, 0), 3);
    }
}
...
static void showDetections(const vector<Rect>& found, Mat& imageData) {
    cvtColor(imageData, imageData, CV_GRAY2BGR); // Convert to RGB to plot colored lines.
    ...
    for (i = 0; i < found_filtered.size(); i++) {
        ...
        rectangle(imageData, r.tl(), r.br(), Scalar(255, 0, 0), 3);
    }
}
...
int main(int argc, char** argv) {
    ...
    hog.winSize = Size(100, 40); // Default training images size as used in paper
    ...
    Mat testImage;
    testImage = imread("../test.jpg", 0);
    detectTest(hog, hitThreshold, testImage);
    imshow("HOG", testImage);
    waitKey(0);
    destroyAllWindows();
    // </editor-fold>
